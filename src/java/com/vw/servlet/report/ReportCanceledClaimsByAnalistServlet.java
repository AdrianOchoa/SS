 ...
 */
@WebServlet("/ReportCanceledClaimsByAnalistServlet")
public class ReportCanceledClaimsByAnalistServlet extends HttpServlet {
    ...
    protected void doPost (HttpServletRequest request, HttpServletResponse response)
        ...
        List<ResultSet> result = 
                new ReportingService().generateCancelledClaimsByAnalistReport(initialDate, finalDate);
        HttpSession session = request.getSession();
        session.setAttribute("result", result);
        session.setAttribute("initialDate", initialDate);
        session.setAttribute("finalDate", finalDate);
        nextPage = "report/reportCanceledClaimsByAnalist.jsp";
